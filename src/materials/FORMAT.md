...
> [!WARNING]
> The checker compares the reading to the body. Everything that is not kanji, which is kana, Latin
> letters, digits, spaces and punctuation, has to appear in the reading exactly and in order. A
> katakana word has one transcription and is checked directly. Kanji needs a dictionary, so
> `GLOSSARY` in `scripts/check-material.mjs` holds the terms that have been got wrong before. Add to
> it whenever one gets through.
